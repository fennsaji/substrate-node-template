...
Substrate](https://github.com/paritytech/substrate/tree/master/bin/dgel-node)
upstream
...
```sh
./target/release/dgel-node --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/dgel-node purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/dgel-node -ldebug --dev
```
...
$ ./target/release/dgel-node --dev --base-path ./my-chain-state/
