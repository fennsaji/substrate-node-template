...
pub mod pallet {
	...
	pub type CandidatesMetadata<T: Config> = 
		StorageMap<_, Twox64Concat, Identifier, URL, OptionQuery>;
	...
	pub enum Event<T: Config> {
		...
		CandidateAdded { identifier: Identifier, deposit: BalanceOf<T> },
		CandidateRemoved { account_id: T::AccountId },
		MetadataUpdated { identifier: Identifier, agent_url: URL},
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn register_candidate(origin: OriginFor<T>, identifier: Identifier) -> DispatchResultWithPostInfo {
			T::RegisterOrigin::ensure_origin(origin)?;

			// ensure we are below limit.
			let length = <Candidates<T>>::decode_len().unwrap_or_default();
			let account_id = Self::get_account_id(&identifier)?;
			...
			Self::deposit_event(Event::CandidateAdded { identifier, deposit });
			Ok(Some(T::WeightInfo::register_as_candidate(current_count as u32)).into())
		}

		#[pallet::weight(T::WeightInfo::register_as_candidate(T::MaxCandidates::get()))]
		pub fn update_metadata(origin: OriginFor<T>, identifier: Identifier, agent_url: URL) -> DispatchResult {
			T::RegisterOrigin::ensure_origin(origin)?;
			
			// TODO: Add it back after LSP integration in admin 
			// let account_id = Self::get_account_id(&identifier)?;
			// let candidates: BoundedVec<CandidateInfo<T::AccountId, BalanceOf<T>>, T::MaxCandidates> = Self::candidates();
			// let candidate_exists = candidates.iter().any(|c| c.who == account_id);
			// ensure!(candidate_exists, Error::<T>::NotCandidate);

			CandidatesMetadata::<T>::insert(identifier,agent_url);

			Self::deposit_event(Event::MetadataUpdated { identifier, agent_url });
		...
		#[pallet::weight(T::WeightInfo::leave_intent(T::MaxCandidates::get()))]
		pub fn remove_candidate(origin: OriginFor<T>, identifier: Identifier) -> DispatchResultWithPostInfo {
			T::RegisterOrigin::ensure_origin(origin)?;
			ensure!(
				Self::candidates().len() as u32 > T::MinCandidates::get(),
				Error::<T>::TooFewCandidates
			);
			let who = Self::get_account_id(&identifier)?;
			let current_count = Self::try_remove_candidate(&who)?;
