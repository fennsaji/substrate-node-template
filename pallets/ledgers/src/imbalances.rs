...
impl<T: Config, GetCurrencyId: Get<T::CurrencyId>> Imbalance<TokenBalance> for PositiveImbalance<T, GetCurrencyId> {
...
impl<T: Config, GetCurrencyId: Get<T::CurrencyId>> Imbalance<TokenBalance> for NegativeImbalance<T, GetCurrencyId> {
...
impl<T: Config, GetCurrencyId: Get<T::CurrencyId>> Drop for PositiveImbalance<T, GetCurrencyId> {
	...
	fn drop(&mut self) {
		...
		let currency_code = super::Pallet::<T>::get_ccy_code(&currency_id).unwrap_or_default();
		TotalIssuance::<T>::mutate(currency_code, |v| *v = v.saturating_add(self.0));
	}
}
...
impl<T: Config, GetCurrencyId: Get<T::CurrencyId>> Drop for NegativeImbalance<T, GetCurrencyId> {
	...
	fn drop(&mut self) {
		let currency_id = GetCurrencyId::get();
		let currency_code = super::Pallet::<T>::get_ccy_code(&currency_id).unwrap_or_default();
		TotalIssuance::<T>::mutate(currency_code, |v| *v = v.saturating_sub(self.0));
	}
}
