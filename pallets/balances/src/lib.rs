...
impl Memo {
	...
	/// Now adding only Memo length check to avoid extreme high length memo due to fee less
	/// transaction
...
pub mod pallet {
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn transfer(
			...
			let dest = Self::get_account_id(&dest)?;
		...
		pub fn set_balance(
			...
			ensure_root(origin)?;
			let who = Self::get_account_id(&who)?;
		...
		pub fn force_transfer(
			...
			ensure_root(origin)?;
			let source = Self::get_account_id(&source)?;
			let dest = Self::get_account_id(&dest)?;
			<Self as Currency<_>>::transfer(
				&source,
				&dest,
	...
	pub enum Event<T: Config<I>, I: 'static = ()> {
		...
		Transfer(Identifier, Identifier, T::Balance),
		...
		BalanceBurned(Identifier, T::Balance),
...
impl<T: Config<I>, I: 'static> GenesisConfig<T, I> {
	...
	pub fn build_storage(&self) -> Result<sp_runtime::Storage, String> {
		<Self as BuildGenesisConfig<T, I>>::build_storage(self)
	}
	...
	pub fn assimilate_storage(&self, storage: &mut sp_runtime::Storage) -> Result<(), String> {
		<Self as BuildGenesisConfig<T, I>>::assimilate_storage(self, storage)
	}
}
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	fn get_account_id(identifier: &Identifier) -> Result<T::AccountId, DispatchError> {
		Ok(T::DidResolution::get_account_id(identifier)
			.ok_or_else(|| Error::<T, I>::DIDDoesNotExist)?)
	}
	...
	/// Check if the AccountId is mapped to some Identifier in our system
